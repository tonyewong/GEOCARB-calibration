...
## parameterSetup.R
##
## set up for sampling the time series parameters from multivariate normal
##
...
par_calib0 <- par_const_calib0
par_fixed0 <- par_const_fixed0

parnames_const_calib <- as.character(input$parameter[ind_const_calib])
parnames_const_fixed <- as.character(input$parameter[ind_const_fixed])
parnames_time_calib <- as.character(input$parameter[ind_time_calib])
parnames_time_fixed <- as.character(input$parameter[ind_time_fixed])
...
if (length(ind_time_calib)>0) {ind_time_calib <- 1:length(ind_time_calib)
} else                        {ind_time_calib <- NULL}
if (length(ind_time_fixed)>0) {ind_time_fixed <- 1:length(ind_time_fixed)
...
const_names_in <- c( parnames_const_calib[ind_const_calib],
                     parnames_const_fixed[ind_const_fixed] )
...
time_names_in <- c(unique(parnames_time_calib[ind_time_calib]), unique(parnames_time_fixed[ind_time_fixed]))
